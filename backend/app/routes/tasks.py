    ...
    try:
        ...
        if due_before:
            try:
                ...
                return (
                    jsonify({"error": "Invalid due_before format. Use YYYY-MM-DD"}),
                    400,
                )
        ...
        if due_after:
            try:
                ...
                return (
                    jsonify({"error": "Invalid due_after format. Use YYYY-MM-DD"}),
                    400,
                )
...
def update_task(task_id):
    ...
    try:
        ...
        if not can_modify_task(task, user_id):
            return (
                jsonify({"error": "You do not have permission to update this task"}),
                403,
            )
        ...
        if "title" in data:
            ...
            if len(title) < 3:
                return (
                    jsonify({"error": "Task title must be at least 3 characters"}),
                    400,
                )
            task.title = title
        ...
        if "assigned_to_id" in data:
            ...
            if assigned_to_id:
                ...
                if not has_project_access(task.project_id, assigned_to_id):
                    return (
                        jsonify({"error": "Assigned user is not a project member"}),
                        400,
                    )
...
def delete_task(task_id):
    ...
    try:
        ...
        if not can_modify_task(task, user_id):
            return (
                jsonify({"error": "You do not have permission to delete this task"}),
                403,
            )
    ...
    try:
        ...
        if parent_id:
            ...
            if str(parent_comment.task_id) != task_id:
                return (
                    jsonify({"error": "Parent comment belongs to different task"}),
                    400,
                )
        ...
        comment_dict = {
            ...
            "parent_id": str(new_comment.parent_comment_id)
            if new_comment.parent_comment_id
            else None,
        }

        return (
            jsonify({"message": "Comment added successfully", "comment": comment_dict}),
            201,
        )
