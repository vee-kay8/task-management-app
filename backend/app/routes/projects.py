...
def create_project():
    ...
    try:
        ...
        return (
            jsonify(
                {"message": "Project created successfully", "project": project_dict}
            ),
            201,
        )
...
def get_project(project_id):
    ...
    try:
        ...
        task_counts = {
            ...
            "in_progress": len(
                [t for t in all_tasks if t.status.value == "IN_PROGRESS"]
            ),
...
def update_project(project_id):
    ...
    try:
        ...
        has_access, member = has_project_access(
            project_id, user_id, required_roles=["ADMIN", "MANAGER"]
        )

        if not has_access:
            return (
                jsonify({"error": "You do not have permission to update this project"}),
                403,
            )
        ...
        if "name" in data:
            ...
            if len(name) < 3:
                return (
                    jsonify({"error": "Project name must be at least 3 characters"}),
                    400,
                )
            project.name = name
...
def delete_project(project_id):
    ...
    try:
        ...
        has_access, member = has_project_access(
            project_id, user_id, required_roles=["ADMIN"]
        )
...
def add_member(project_id):
    ...
    try:
        ...
        has_access, member = has_project_access(
            project_id, current_user_id, required_roles=["ADMIN", "MANAGER"]
        )
        ...
        new_member = ProjectMember(
            project_id=project_id, user_id=new_user_id, role=role
        )
...
def remove_member(project_id, member_user_id):
    ...
    try:
        ...
        has_access, member = has_project_access(
            project_id, current_user_id, required_roles=["ADMIN"]
        )
        ...
        if (
            member_to_remove.role == GlobalUserRole.ADMIN
            and str(project.owner_id) == member_user_id
        ):
