...
class User(db.Model):
    ...
    password_hash = db.Column(
        db.String(255), nullable=False, comment="Bcrypt hashed password"
    )

    # Full Name - user's display name
    full_name = db.Column(db.String(255), nullable=False, comment="User full name")

    # Avatar URL - profile picture (S3 URL in Phase 4)
    avatar_url = db.Column(
        db.String(500), nullable=True, comment="Profile picture URL"  # Optional field
    )
    ...
    is_active = db.Column(
        db.Boolean, default=True, nullable=False, comment="Account enabled/disabled"
    )

    email_verified = db.Column(
        db.Boolean, default=False, nullable=False, comment="Email verification status"
    )
    ...
    reported_tasks = db.relationship(
        "Task", foreign_keys="Task.reporter_id", backref="reporter", lazy="dynamic"
    )

    # Project memberships (through project_members table)
    project_memberships = db.relationship(
        "ProjectMember", backref="user", lazy="dynamic", cascade="all, delete-orphan"
    )
