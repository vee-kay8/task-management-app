...
class JSONType(TypeDecorator):
    ...
    Uses JSONB for PostgreSQL (better performance + indexing)
...
class Task(db.Model):
    ...
    title = db.Column(db.String(500), nullable=False, comment="Task title/summary")

    # Description - detailed information
    # Supports markdown formatting
    description = db.Column(
        db.Text, nullable=True, comment="Task detailed description (supports markdown)"
    )
    ...
    completed_at = db.Column(
        db.DateTime(timezone=True), nullable=True, comment="Completion timestamp"
    )
    ...
    actual_hours = db.Column(
        db.Numeric(5, 2), nullable=True, comment="Actual time spent in hours"
    )

    # Tags - categories/labels stored as JSON array
    # Example: ["bug", "frontend", "urgent"]
    # Uses JSONB for PostgreSQL (efficient querying with GIN index)
    # Uses JSON for SQLite (testing compatibility)
    tags = db.Column(
        JSONType,  # Custom type that adapts to database
    ...
    attachments = db.relationship(
        "Attachment", backref="task", lazy="dynamic", cascade="all, delete-orphan"
    )
    ...
    __table_args__ = (
        db.Index(
            "idx_task_project_status_position", "project_id", "status", "position"
        ),
    )
